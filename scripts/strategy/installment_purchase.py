...
class InstallmentPurchase:
    def __init__(self, sell_rate=4, buy_rate=-10, black_list=None, options=None):
    ...
    def _get_init_krw(self):
        """매수 금액을 설정"""
        min_krw, max_krw = (6000, 20000)
        result = self._upbit.get_my_total_krw() / 365
        if result < min_krw:
            result = min_krw
        if result > max_krw:
            result = max_krw
    ...
    def run(self):
        ...
        for currency in currency_list:
            ...
            if not self._options['disabled_buy'] \
                ...
                add_buy_krw = self._init_krw // 2
                self.log(currency, '{}% 이하로 손실중. 추가 매수 진행. rate: {} / 금액: {}'.format(self._buy_rate, rate, add_buy_krw))
                self._upbit.buy_market(currency, add_buy_krw)
